...
c         call dblepr("cdfw",-1,cdfw,1)
c         call dblepr("cdfval",-1,cdfval,1)
c         call rexit("error in cdf: lower")
      end if    
...
c+++++++ check if the user has requested an interrupt
         ...
         tmp1=0.d0
         do j=1,ptf
            tmp1=tmp1+xtf(i,j)*betatf(j,j)
         end do
         tmp3=(b(i)-tmp1)/dsqrt(sigma2b)

         loglik=loglik+dnrm(b(i),tmp1,sqrt(sigma2b),1)
         ...
         if(tmp3.gt.4.0)then
            ...
            tmp2=cdfnorm(b(i),tmp1,sqrt(sigma2b),1,0)
         end if
